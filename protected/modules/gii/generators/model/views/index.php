...
$form=$this->beginWidget('CCodeForm', array('model'=>$model)); ?>
	...
	<div class="row model-class">
		<?php echo $form->label($model,'modelUploadColumns',array()); ?>
		<?php echo $form->textField($model,'modelUploadColumns', array('size'=>65)); ?>
		<div class="tooltip">
			This is the name of the fields upload not in database to be generated (e.g. <code>files, images</code>).
		</div>
		<?php echo $form->error($model,'modelUploadColumns'); ?>
	</div>

	<div class="row model-class" id="moreopt">
		<?php 
		if ($model->tableName != '' && !empty($columns))
			$this->renderPartial('moreoption', array('columns' => $columns, 'model' => $model, 'uploadFields' => $uploadFields));
		?>
