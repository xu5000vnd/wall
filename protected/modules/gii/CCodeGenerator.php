...
class CCodeGenerator extends CController {
	...
	public function actionGetAttributes() {
		...
		$modelUploadColumns = array();
		foreach (get_object_vars($model) as $object_var_key => $object_var_value) {
			if ($object_var_key == 'uploadArray' && !empty($object_var_value)) {
				foreach ($object_var_value as $key => $value) {
					$modelUploadColumns[$key] = $value['uploadType'];
				}
			}
		}
		$this->renderPartial('/admin/get_attributes', array(
			'model' => $model, 
			'action' => $action, 
			'table' => $table, 
			'modelUploadColumns' => $modelUploadColumns, 
			'relation' => $relation
		));
	}
